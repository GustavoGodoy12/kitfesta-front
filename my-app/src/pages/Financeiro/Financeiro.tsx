...
export default function Financeiro() {
  ...
  async function buscar() {
    ...
    try {
      ...
      const lista: Pedido[] = Array.isArray(json?.pedidos) ? json.pedidos : Array.isArray(json) ? json : []
      setPedidos(lista)
    } catch (err) {
      console.error(err)
  ...
  const resumo: Resumo = pedidos.reduce<Resumo>((acc, p) => {
    ...
    return acc
  }, { totalFaturado: 0, totalPedidos: 0, ticketMedio: 0, totalEntregues: 0, totalPendentes: 0, totalRetirada: 0 })
  ...
  const porPagamentoData: ChartBarItem[] = Object.entries(porPagamento)
    ...
    .map(([label, value]) => ({
      label,
      value,
      color: CORES_PAGAMENTO[label] ?? '#94a3b8',
    }))
          ...
          <>
            ...
            <CardsGrid>
              ...
              <SummaryCard $color="#f59e0b">
                ...
                <CardValue $color="#f59e0b">{resumo.totalPendentes}</CardValue>
                <CardSub>aguardando entrega</CardSub>
              </SummaryCard>
            </CardsGrid>
